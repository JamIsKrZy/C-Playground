...
struct PIDS_ID{
    pid_t pids[PROCESS_POOL_LIMIT];
    size_t index_rep[PROCESS_POOL_LIMIT];
    int readfd[PROCESS_POOL_LIMIT];
};


static struct PIDS_ID create_pids(){
    struct PIDS_ID pids = {0};
    for (size_t i = 0; i < PROCESS_POOL_LIMIT; i++) {
        pids.pids[i] = -1;
    }
    return pids;
}
...
void set_file_actions(
    ...
    posix_spawn_file_actions_adddup2(file_actions, writefd, STDOUT_FILENO);
    posix_spawn_file_actions_addclose(file_actions, readfd);
...
ProcessResults process_executable(
    ...
    struct PIDS_ID pidsList = create_pids();
    ...
    while (remaining_executables != 0)
    {
        ...
        for (size_t i = 0; i < PROCESS_POOL_LIMIT; i++)
        {
            if(pidsList.pids[i] != -1){ continue;}

            // checks if there are any next executables
            //  to process
            ...
            char *args[] = {exe, NULL};
            create_process(exe, pidsList.pids + i, &file_actions, &args);
        ...
        for (size_t i = 0; i < PROCESS_POOL_LIMIT; i++)
        {
            ...
            char *name = get_filename_from_list(data, pidsList.index_rep[i]);
            process_signal = wait_for_PID_result(name, pidsList.pids + i);

            if(process_signal.process_finished == true){
                result.res[pidsList.index_rep[i]] = process_signal.return_value;
...
/**
 * void* recieves the file descriptors
 */
void* ipc_handle(void *handle){
    struct IpcHandle handle = *(struct IpcHandle*)handle;
